...
! ... read dbound.###:
      ...
      if(AF(1:1).eq.'d') then
       ...
       if(ichw.eq.1 .and. .not.allocated(S_chan)) then
        allocate(S_chan(nchan)); S_chan=0.d0
       end if
       ! ... find given state:
...
! ... transfer the solution to the p-functions:
      ...
      Do ich = 1,nchan
       ...
       if(ichw.eq.1)  then
        if(S.gt.S_chan(ich)) S_chan(ich)=sqrt(s) 
       end if
...
!  ... output bsw-file:
    ...
    2 Continue
      ...
      if(ichw.eq.1) then
       open(pri,file='Channel_weights')
       Do i = 1,nchan
        write(pri,'(i5,f10.3)')  i, S_chan(i)
       End do 
      end if
