...
! ... define Ak coefficients:
      ...
      Do j = 1,ncase
       ...
       Do k = kmin, kmax,2; kk = k+k+1
        ...
        S =  S * Z_6j(ki1,kj1,kk,mj1,mi1,JT1);  if(S.eq.0.d0) Cycle 
        S =  S * Z_6j(ki2,kj2,kk,mj2,mi2,JT2);  if(S.eq.0.d0) Cycle 
        S =  S * Z_6j(ki1,kj1,kk,JP2,JP1,2);    if(S.eq.0.d0) Cycle 
        S =  S * Z_6j(ki2,kj2,kk,JP2,JP1,2);    if(S.eq.0.d0) Cycle 

        T =  TR(i)*TR(j) + TI(i)*TI(j)
...
! ... Q coefficients:
      ...
      if(ion.gt.0.and.itr1.eq.itr2) then
       ...
       Allocate(QR(0:llmax,ne),QI(0:llmax,ne))
      
       QI = 0.d0; QR = 0.d0
      
       Do i = 1,ncase; ie=ipe(i)
        if(l1(i).ne.l2(i)) Cycle 
        if(jk1(i).ne.jk2(i)) Cycle
        l=l1(i)
        S = jjpar(i)
        sigma=-2.d0*CP(l,ie); sr=cos(sigma); si = sin(sigma)
        QR(l,ie) = QR(l,ie) + S*(TR(i)*sr - TI(i)*si)
        QI(l,ie) = QI(l,ie) + S*(TR(i)*si + TI(i)*sr)
       End do

       write(pri,'(/a/)') 'Q(k) - coefficients' 
       Do ie = 1,ne
        write(pri,'(a,f10.6/)') 'EK =',ev(ie) 
        Do k=0,llmax
         write(pri,'(i3,3E15.5)') k,QR(k,ie),QI(k,ie),CP(k,ie)
        End do
       End do
...
! ... angle-integrated cross-sections:
      ...
      if(ion.eq.0.or.itr1.ne.itr2) then
       ...
       Do ie = 1,ne
        S = 8.d0*(ek(ie)-etarg(itr1))*JT1     
        Ak(:,ie) = Ak(:,ie) / S
        A0 = Ak(0,ie)
        A1 = Ak(1,ie)
        sig  = 4*PI * Snorm * A0
        sigM = 4*PI * Snorm * (A0 - A1/3.d0)
        write(nur,'(f10.5,2e15.5,f15.6)')  (ev(ie)-etarg(itr1))*Ry,sig,sigM,ev(ie)
       End do
...
!======================================================================
      Subroutine inf_sec_dif_JK
!======================================================================
!     provides screen information about sec_dif_JK utility
!----------------------------------------------------------------------
...
' Call as:                                                   ',&
'                                                            ',&
'        sec_dif_JK itr1=ii itr2=jj i16=-1|0|. ifano=0|1     ',&
'                   ek1=...  ek2=...                         ',&
'                   gmin=... gmax=... gstep=...              ',&
'                   dcs=0|1  tdone=0|1                       ',&
'                                                            ',&
