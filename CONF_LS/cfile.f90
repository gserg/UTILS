...
!                                                           input data:
      ...
      if(iarg.lt.5) then
        ...
        Call GET_COMMAND_ARGUMENT(1,AN)
        Call GET_COMMAND_ARGUMENT(2,AF); read(AF,*) nn
        Call GET_COMMAND_ARGUMENT(3,AF); read(AF,*) jj
        Call GET_COMMAND_ARGUMENT(4,BF)
        Call GET_COMMAND_ARGUMENT(5,AF); read(AF,*) eps_c
      end if
...
! ... read solution:
    ...
    5 read(nuj,'(a)',end=9) AS
      ...
      i = INDEX(AS,'=');  read(AS(i+1:),*) j
      i = INDEX(AS,'=',BACK=.TRUE.);  read(AS(i+1:),*) nj
      ...
      Close(iout)
       ...
       if(Icheck_file(AN).eq.1) then
        ...
        if(AN.ne.BF) then
         ...
         i = SYSTEM(AS)
!         write(AS,'(a,a,a,a)') 'copy ',trim(AN),' ',trim(BF)
!         i = SYSTEM(AS)
        end if
       end if
