...
!      thresholds_pert  -  LS pertubers information
!
!     OUTPUT FILES:
!
...
! ... read LS-target if any
      ...
      if(Icheck_file(AF_pert).ne.0) then
       ...
       Allocate(index_kpert(nlsp,1)); index_kpert=0
      end if
...
! ... read bound.nnn, ubound.nnn, or pol.nnn:
      ...
      iform = 0
      if(len_trim(Afb).ne.0) then

       if(Afb(1:1).eq.'u') then
        iform=10; Open(nub,file=AFb,form='UNFORMATTED')
       end if
       if(Afb(1:1).eq.'b') then
        iform=20;  Open(nub,file=AFb)
       end if
       if(Afb(1:1).eq.'p') then
        iform=30;  Open(nub,file=AFb)
       end if
      ...
      if(iform.eq.0) then
       write(*,*) 'Can not find solutions file: ubound.nnn, bound.nnn, pol.nnn ...'
       Stop ' '
      end if
