...
!=======================================================================
      ...
      Integer :: ke=1000, ne=0, me, lform = 0
...
! ... choose the partial waves:
  ...
  10  write(*,*) 'Number of partial waves:'
      read(*,*,err=10) nphot
      Allocate(IP_phot(nphot), NU_phot(nphot), E_thresh(nphot))
  20  write(*,*) 'Their indexes according to file target:'
      read(*,*,err=20) IP_phot
...
! ... choose the work:
    ...
    1 Continue
      ...
      write(*,*) '1  - total and partial cross sections'
      write(*,*) '2  - channel cross sections'
      write(*,*) '3  - ionic-state cross sections (in separate files)'
      write(*,*) '4  - beta-parameters for given taret states'
      write(*,*) '5  - dipole matrix elements for given partial wave and channel'
      write(*,*) '6  - delete energies (provide delete_list, bsr_phot.nnn -> bsr_phot.nnn_del)'
      write(*,*) '7  - check archives (bsr_phot.nnn -> bsr_phot.nnn_corrected)'
      write(*,*) '8  - clean archives (provide energy_list, bsr_phot.nnn -> bsr_phot.nnn_cleaned)'
      write(*,*) '9  - dipole matrix elements, range of channels'
      write(*,*) '0  - exit'
...
!=======================================================================
    ...
    1  read(nu,'(2d15.8,4i5)',end=2) e1,e2,nopen,nwt,ikm
     ...
     2 Continue
      ...
      End do  ! over iphot
!----------------------------------------------------------------------
      write(AF_res,'(a)') 'Total_cs'

      Open(nur,file=AF_res)

      if(nphot.eq.1)  then
      write(nur,'(T8,a,T24,a,3(6x,a,2x))') &
       'E_phot_eV', 'E_elc_Ry', 'Total_L', 'Total_V', '(in Mb)'
      else
      write(nur,'(T8,a,T24,a,T43,a,T71,200(a,i3.3,19x) )') 'E_phot_eV','E_elc_Ry',&
        'Total c.s. (Mb)',('partial_',j,j=1,nphot)
      write(nur,'(T39,200(a,14x,a,14x))') ('L','V',j=1,nphot+1)
      end if
...
!       if(CL.gt.1000) IP_energy(ie) = 1
       ...
       if(nphot.eq.1) then
        write(nur,'(f16.8,f15.6,20E15.5)') ev(ie),e(ie), CL,CV  
       else
        write(nur,'(f16.8,f15.6,20E15.5)') ev(ie),e(ie), CL,CV, &  
          (CSR(ie,j),CSV(ie,j),j=1,nphot)
       end if
...
!=======================================================================
    ...
    1  read(nu,'(2d15.8,4i5)',end=2) e1,e2,nopen,nwt,ikm
     ...
     2 Continue
      ...
      write(AF_res,'(3(a,i3.3))')  'channels_',klsp,'_',ich1,'_',ich2
      Open(nur,file=AF_res)

      if(allocated(term)) deallocate(term); Allocate(term(kch))

      Do i=1,kch
       if(coupling.eq.'LS') then
        write(at,'(a,a)') 'k',AL(lch(klsp,i),1)
       elseif(coupling.eq.'JK') then                  ! 2K+1  ?
        if(mod(jkch(klsp,i),2).eq.1) &
        write(at,'(a,a,a,i1,a)') 'k',AL(lch(klsp,i),1),'[',(jkch(klsp,i)-1)/2,']' 
        if(mod(jkch(klsp,i),2).eq.0) &
        write(at,'(a,a,a,i1,a)') 'k',AL(lch(klsp,i),1),'[',jkch(klsp,i)-1,'/2]' 
       else
        if(mod(jkch(klsp,i),2).eq.0) &                ! 2J   ?
        write(at,'(a,a,a,i1,a)') 'k',AL(lch(klsp,i),1),'[',jkch(klsp,i)/2,']' 
        if(mod(jkch(klsp,i),2).eq.1) &
        write(at,'(a,a,a,i1,a)') 'k',AL(lch(klsp,i),1),'[',jkch(klsp,i),'/2]' 
       end if
       term(i) = at
      ...
      write(nur,'(T36,150(a,i3,a,i3,2x,a7,3x))') &
                ('target',iptar(klsp,i),' chan.',i, trim(term(i)),i=ich1,ich2)

      write(nur,'(T8,a,T23,a,T40,200(a,14x,a,14x))') &
                'E_phot_eV', 'E__elc_Ry',('L','V',i=ich1,ich2), '(in Mb)'

      Do i=1,ne; ie=IPE(i)
       write(nur,'(f16.8,f15.6,200E15.5)') ev(ie), e(ie), &
...
! ... output in separate files:
      ...
      Do it = 1,ntarg
       ...
       write(AF,'(a,i3.3)') 'target_cs_',it
       Open(nur,file=AF)

       write(nur,'(T8,a,T24,a,T38,a,8x,a,8x,a)') &
        'E_phot_eV','E_elc_Ry','sigma_L','sigma_V','(in Mb)'

       Do i=1,ne; ie=IPE(i); if(IP_energy(ie).eq.1) Cycle
        if(Atarg(ie,it).eq.0.d0) Cycle
        write(nur,'(f16.8,f15.6,2E15.5)') ev(ie), e(ie)-etarg(it), &
...
! ... sums:
      ...
      write(*,*) AF_states, Icheck_file(AF_states)
...
!=======================================================================
   ...
   1  read(nu,'(2d15.8,3i5)',end=2) e1,e2,nopen,nwt,ikm
    ...
    2 Continue
      ...
      write(AF_res,'(a,i3.3,a,i3.3)') 'dme_',klsp,'_',ich
      ...
      write(nur,'(T48,a,i3,a,i3,3x,a)') &
                'target',iptar(klsp,ich),'   channel',ich, trim(at)

      write(nur,'(T8,a,T24,a,8x,a,11x,a,11x,a,11x,a)')  &
                'E_phot_eV', 'E_elc_Ry', 'dr_L', 'di_L', 'dr_V', 'di_V' 

      Do ie=1,ne
       write(nur,'(f16.8,f15.6, 4E15.5)') ev(ie), e(ie), &
           dd(1,ie), dd(3,ie), dd(5,ie), dd(7,ie)
      End do

      write(*,'(/a,a/)') 'Output file: ', trim(AF_res)

      End Subroutine Dipole_me


!=======================================================================
      Subroutine Dipole_me_new
!=======================================================================
      ...
      write(*,*) 'partial wave and channels range: '
      read (*,*) iphot,ich1,ich2
      ...
      if(allocated(dd)) deallocate(dd); Allocate(dd(kch,4,ne)); dd = 0.d0
      if(allocated(term)) deallocate(term); Allocate(term(kch))
   ...
   1  read(nu,'(2d15.8,3i5)',end=2) e1,e2,nopen,nwt,ikm
      ...
      if(ie.eq.0) Stop 'Dipole_me: problems with energies'
       ...
       dd(ich1:ich2,1,ie) = DLR(ich1:ich2)
       dd(ich1:ich2,2,ie) = DLI(ich1:ich2)
       dd(ich1:ich2,3,ie) = DVR(ich1:ich2)
       dd(ich1:ich2,4,ie) = DVI(ich1:ich2)

      go to 1
    2 Continue

      write(AF_res,'(3(a,i3.3))') 'dme_',klsp,'_',ich1,'_',ich2

      Open(nur,file=AF_res)

      write(nur,'(T48,150(a,i3,a,i3,3x,a9,28x))') &
                ('target',iptar(klsp,i),' channel',i, term(i),i=ich1,ich2)

      write(nur,'(T7,a,T24,a,250(8x,a,3x))')  &
           'E__phot_eV', 'E_elc_Ry', ('dr_L','di_L','dr_v','di_V',i=ich1,ich2) 

      Do ie=1,ne
       write(nur,'(f16.8,f15.6, 200E15.5)') ev(ie), e(ie), &
            ((dd(i,j,ie),j=1,4),i=ich1,ich2)
      End do

      write(*,'(/a,a/)') 'Output file:  ',trim(AF_res)


      End Subroutine Dipole_me_new
...
!----------------------------------------------------------------------
      ...
      write(AF_res,'(a,i3.3,a,i3.3)') 'beta_',it1,'_',it2
      Open(nur,file=AF_res)
      write(nur,'(T7,a,T24,a,T43,200(a,i3.3,19x) )') 'E_phot_eV','E_elc_Ry',&
        ('target_',it,it=it1,it2)
      write(nur,'(T40,200(a,14x,a,14x))') ('L','V',it=it1,it2)
...
! ... target loop:
       ...
       write(nur,'(f16.8,f15.6,20E15.5)') ev(ie),e(ie), &
