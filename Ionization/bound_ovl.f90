!--------------------------------------------------------------------
!     w.nnn, target, target_ps  -->  bound_ovl  (list of ion-target weights)
!--------------------------------------------------------------------
      Use target
      Use channels
...
!------------------------------------------------------------------------------------
      ...
      write(*,'(a)') &
'                                                                                      ',&
'     BOUND_OVL provides decomposition of pseudostates in terms of ionic target states ',&
'                                                                                      ',&
'                 target, target_ps, w.nnn  -->  bound_ovl                             ',&
'                                                                                      ',&
'     Call as:    bound_ovl                                                            ',&
'                                                                                      ',&
'     Calculations should be done in the folder of BSR calculations for the            ',&
'     ejected electron (scattering on ion)                                             ',&
'                                                                                      '
      Stop ' '                                                                    
      end if
...
! ??? re-normalization to 1 
       ...
       End do   ! over is in w.nnn (pseudostates with given LSP)
...
! ... check if we found all needed information:
      ...
      Do i=1,nps
       ...
       write(*,*) i,LPS(i), SPS(i), PPS(i),Eps(i),' -  pseudo-state not found'
      End do
